...
const Toast: React.FC = () => {
  ...
  const getToastStyles = (type: string) => {
    switch (type) {
      ...
      case 'warning':
        return 'bg-warning text-black border-2 border-warning';
      default:
  ...
  return (
    <div className="fixed bottom-20 md:bottom-4 right-4 z-50 space-y-2">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={`flex items-center gap-3 px-4 py-3 rounded-sm shadow-md animate-slideIn ${getToastStyles(toast.type)}`}
          role="alert"
        >
          {getIcon(toast.type)}
